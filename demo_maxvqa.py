...
#     # Use val-maxwell since it's the only dataset you have now
#     dopt = opt["data"]["val-maxwell"]["args"]
...
# Initialize DOVER-based Fast VQA encoder
fast_vqa_encoder = DOVER(**opt["model"]["args"]).to(device)
fast_vqa_encoder.load_state_dict(torch.load("/home/ubuntu/MaxVQA/DOVER/pretrained_weights/DOVER.pth"), strict=False)
...
dim_means = [0.64816874, 0.7389499, 0.72287375, 0.7463802, 0.8296112, 0.7190873, 0.6247339, 0.54232854, 0.629289, 0.5732389,
             0.69551915, 0.49694163, 0.68725526, 0.60615003, 0.88730884, 0.5536831]
dim_stds = [0.16527903, 0.1164713, 0.12123751, 0.13628706, 0.12584831, 0.11652958, 0.15417372, 0.18802027, 0.16696042, 0.15306316,
            0.153056, 0.1480174, 0.1273537, 0.16313092, 0.07106569, 0.1876018]
...
def query_gpt(prompt, scores):
    combined_prompt = f"{prompt}\nHere are the scores:\n{scores}\nBased on these scores, can you answer the question?"
    input_ids = gpt_tokenizer.encode(combined_prompt, return_tensors="pt")
    output = gpt_model.generate(input_ids, max_length=300, num_return_sequences=1)
    return gpt_tokenizer.decode(output[0], skip_special_tokens=True)

def interaction(text_input, video_input):
    if video_input is not None:
        scores = inference(video_input)
        llm_response = query_gpt(text_input, scores)
    else:
        llm_response = "Please upload a video for analysis."

    return llm_response

iface = gr.Interface(
    fn=interaction,
    inputs=[gr.inputs.Textbox(label="Your question"), gr.inputs.Video(source="upload")],
    outputs="text",
    title="Interactive MaxVQA Analysis with GPT"
)
